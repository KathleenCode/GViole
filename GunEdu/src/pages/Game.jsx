...
export default function Game() {
  const [inptvl, setInptvl] = useState(0);
  const [info, setInfo] = useState({});
  // const [show, setShow] = useState("Show More")
  const [loading, setLoading] = useState(true);

  const sdg_data = useLoaderData();
  const findInfo = sdg_data.find((i) => i.id === info);
  ...
  useEffect(() => {
    const getOne = async (id) => {
      loading;
      try {
        const resp = await fetch("http://localhost:9000/SDGs/" + id);
        if (resp.status !== 200) {
          throw new Error("could not fetch");
        }
        const sdg = await resp.json();
        console.log(sdg);
        setInfo(sdg);
      } catch (err) {
        console.log(err);
  ...
  return (
    <>
      <section className="game">
        <div className="gam">
          <h1>SDG Quiz</h1>
          <h2>How well do you know the SDG's?</h2>
          <h3>Play this game to know !</h3>
          <h4>
            Input a number. <br /> <span>Hint: from one to seventeen</span>{" "}
            <br />
            <span>There are only seventeen SDGs</span>
          </h4>
          <input
            type="number"
            min="1"
            max="17"
            value={inptvl}
            onChange={(e) => setInptvl(e.target.value)}
          />
          <div className="ans">
            <p>
              <span>Number:</span> &nbsp;
      ...
      {loading ? <p>Wait a minute. Loading ...</p> : null}
    </>
  );
}

export async function gload() {
  try {
    const resp = await fetch("http://localhost:9000/SDGs");
    if (!resp.ok) {
